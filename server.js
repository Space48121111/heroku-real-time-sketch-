...
function transmitMessages() {
  ...
  // socket.emit only to individual users.
...
io.on('connection', (socket) => {
  ...
  // for one specific user socket local to this section i.e. banned account
  socket.on('chitChat', (message) => {
    convo.push(message);
    // chatting part
    // get the new message and update the screen
    transmitMessages();
  })
